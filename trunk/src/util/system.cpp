...
const char * const BITCOIN_CONF_FILENAME = "canuckcoin.conf";
...
#else
    const char* pszModule = "canuckcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Canuckcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Canuckcoin";
#else
    // Unix
    return pathRet / ".canuckcoin";
#endif
#endif
}
