# PSBT Howto for Canuckcoin Core

Since Canuckcoin Core 0.17, an RPC interface exists for Partially Signed Canuckcoin
...
PSBT is an interchange format for Canuckcoin transactions that are not fully signed
...
Overall, the construction of a fully signed Canuckcoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid Canuckcoin transaction (in network format)
...
#### Multisig with multiple Canuckcoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Canuckcoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `canuckcoin-cli` is used.
